...
This month, one layer down: **Agent Co-op** — a local-first coordination protocol for supervised terminal coding agents from *different* providers, because the unsolved boundary is between agents, not inside them. Fenced claim lanes with leases and fencing tokens, exact-question channels, structured handoffs, reviewer ≠ owner, and completion that re-hashes its own evidence receipts. ~7.6k lines of dependency-free Python behind 558 passing tests, built in three phases — each design put through multi-round adversarial review by a rival model *before* any code, and the round that earned its keep found a live defect in already-shipped code that 295 passing tests had missed. Status, by the project's own ladder: **tested, not exercised**. A supervised multi-provider run has since happened, and it earned its keep by failing usefully: agents sat idle with actionable work in their own inboxes, waiting to be re-invoked by an orchestrator that attended mode doesn't have. That idle *was* the unsolved problem, hidden while a human was driving — so the acceptance gate moved to autonomous coordination under supervision, and the headline claim stays unmade until that runs.
