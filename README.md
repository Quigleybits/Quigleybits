...
This month, one layer down: **Agent Co-op** — a local-first coordination protocol for terminal coding agents from *different* providers, because the unsolved boundary is between agents, not inside them. Fenced claim lanes with leases and fencing tokens, exact-question channels, structured handoffs, reviewer ≠ owner, and completion that re-hashes its own evidence receipts. ~28k lines of dependency-free Python behind 1,334 passing tests — each design put through multi-round adversarial review by a rival model *before* any code, and the round that earned its keep found a live defect in already-shipped code that 295 passing tests had missed. The first supervised run earned its keep by failing usefully: agents sat idle with actionable work in their own inboxes, waiting to be re-invoked by an orchestrator that attended mode didn't have. That idle *was* the unsolved problem, hidden while a human was driving — so the acceptance gate moved to autonomous coordination, and a human board write mid-run now *fails* it. It has since run that way end-to-end: first on itself across three providers, then on a repo that isn't its own — fixing a real bug there, zero mid-run human writes, that repo's own test gate green on the result. That has now hardened into a **pre-registered batch** — ten trials frozen in a plan before the first run, across four repos that aren't its own and three providers, one of them a red arm that had to refuse and stop honestly — landing **8/10 correct terminal states, zero critical protocol violations**. The batch paid for itself by catching a live defect in already-shipped code (cold provider spawns overflowing the Windows argv limit), and its two remaining failures were traced by controlled experiment to my own operator harness killing the runner — the measurement apparatus, not the protocol. Status, by the project's own ladder: **tested, and exercised** — existence, not reliability. The ≥95% bar stays unclaimed, it needs a 30–60-run expansion, and every failed trial stays in the denominator — including the retry that failed identically and left the batch paused at 8 of 11.
