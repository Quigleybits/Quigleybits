...
## Currently
<!-- NOW:START -->
Building **agent_os** — a personal agentic operating system, run as a private research workshop: a 5-role team (planner, coder, tester, always-on critic, doc-maintainer) resident in my daily harness, with the orchestrator and paired researchers scoped to the loop repo; a 20-role loop-team design — ~20 specialist roles on file, only 2 resident; and 13 memory systems scored against a 6-level taxonomy for memory that outlives sessions. The bet underneath: **the same model swings widely by harness — the scaffold is where the engineering lives.**

This month, one layer down: **Agent Co-op** — a local-first coordination protocol for terminal coding agents from *different* providers, because the unsolved boundary is between agents, not inside them. Fenced claim lanes with leases and fencing tokens, exact-question channels, structured handoffs, reviewer ≠ owner, and completion that re-hashes its own evidence receipts. ~29k lines of dependency-free Python behind 1,451 passing tests — each design put through multi-round adversarial review by a rival model *before* any code, and the round that earned its keep found a live defect in already-shipped code that 295 passing tests had missed. The first supervised run earned its keep by failing usefully: agents sat idle with actionable work in their own inboxes, waiting to be re-invoked by an orchestrator that attended mode didn't have. That idle *was* the unsolved problem, hidden while a human was driving — so the acceptance gate moved to autonomous coordination, and a human board write mid-run now *fails* it. It has since run that way end-to-end: first on itself across three providers, then on a repo that isn't its own — fixing a real bug there, zero mid-run human writes, that repo's own test gate green on the result. That has now hardened into a **pre-registered batch** — eleven trials frozen in a plan before the first run, across four repos that aren't its own, owner rotation covering all three providers, one of them a red arm that had to refuse and stop honestly — landing **10 of 11 with zero critical protocol violations**, and catching the batch's own product defect on the way: cold provider spawns overflowing the Windows argv limit, closed at runner level. Two further trials were voided as measurement-apparatus interference, adjudicated against kill forensics rather than quietly dropped. Status, by the project's own ladder: **tested, and exercised** — existence, not reliability. The ≥95% bar stays unclaimed — it needs a 30–60-run expansion — and every failed trial stays in the denominator.
...
- **Verification is the bottleneck** — generation has outrun checking; autonomous loops compound only through a three-part ratchet (safety veto, minimum-improvement delta, re-baselining on the same evals), and a maturity ladder that forbids reporting a feature above its lowest failing level — specified / writable / readable / enforced / tested / exercised. "We have it" is not a status.
...
- **[Scosig](https://scosig.com) — a research-intelligence feed scoped to one researcher's signal (code private).** Ran a five-judge blind readability bake-off across six summariser models, then deliberately shipped the *third*-ranked one: the two winners were SLA-less free-tier endpoints, unfit for an unattended cron. The measurement decides the fact; the human decides the ship — and the operational grounds for overruling it were named in advance. When the summariser came up for a cheaper replacement I tried to automate that same judgment with an LLM-as-judge — then calibrated it against the design partner's own earlier blind verdict and threw it out: it picked the incumbent 0/6 where the human verdict had picked it 3/6, so taste stayed with the human and the swap shipped on objective floor metrics instead — 2.6× cheaper at an unchanged quality floor, the old model left wired as a one-env-var rollback. Ranking constants move one variable at a time behind an eval gate (three independent blind judges, quadratic-weighted κ=0.874, before the fusion weights were promoted). When dogfooding with a second researcher exposed an ingest path that crossed tenants, the fix deleted the global code path rather than guarding the caller — making the bug class unrepresentable rather than caught. *Product live; code private.*
- **Portfolio-wide security audit — 61 repos, one adversarially-verified pass (private).** Findings confirmed with live before/after verification rather than taken on the reviewer's word; two git-history purges executed and re-checked; a calibrated disclosure written for the one third-party codebase (stating what *wasn't* reviewed). The cross-cutting lesson — an unattended agent never holds untrusted input, secrets, *and* broad execution at once — became a standing containment rule that now gates my headless-agent fleet. *Findings ledger + writeup on request.*
