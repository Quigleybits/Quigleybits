...
This month, one layer down: **Agent Co-op** — a local-first coordination protocol for supervised terminal coding agents from *different* providers, because the unsolved boundary is between agents, not inside them. Fenced claim lanes with leases and fencing tokens, exact-question channels, structured handoffs, reviewer ≠ owner, and completion that re-hashes its own evidence receipts. 19k lines of dependency-free Python behind 1,068 passing tests in its first twelve days — each design put through multi-round adversarial review by a rival model *before* any code, and the round that earned its keep found a live defect in already-shipped code that 295 passing tests had missed. An early supervised run failed usefully: agents sat idle holding actionable work in their own inboxes, waiting on an orchestrator that attended mode doesn't have — so the acceptance gate moved to autonomous coordination, and that gate has now cleared, agents from different providers carrying one item to `all_done` in 24 autonomous turns with zero mid-run human board mutations. The sharpest call of the build was to *stop measuring*: a fifteen-run cross-provider speed benchmark was killed six hours in, once one pass over the traces showed 99.9% of every run was the providers thinking — coordination contributes about two seconds to a thirty-two-minute run, inside a run-to-run spread of eighteen minutes — so two finished optimisation batches stay deliberately unpromoted rather than promoted on a number that was never measuring them. Status, by the project's own ladder: a complete, well-tested protocol with one day of real-world evidence. The next gate is written so it can genuinely fail — one task on a repo that isn't Co-op's own, launched from outside it, the human walks away, and the target repo's own acceptance command decides.
