...
## Currently
<!-- NOW:START -->
Building **agent_os** — a personal agentic operating system, run as a private research workshop: an 8-role agent team (planner, paired researchers, coder, tester, always-on critic, doc-maintainer) deployed in my daily harness; a loop runtime with ~20 specialist roles on file but only 2 resident; and a 6-level taxonomy for memory that outlives sessions. The bet underneath: **the same model's task success can double across harnesses — the scaffold is where the engineering lives.**

This month, one layer down: **Agent Co-op** — a local-first coordination protocol for terminal coding agents from *different* providers, because the unsolved boundary is between agents, not inside them. Fenced claim lanes with leases and fencing tokens, exact-question channels, structured handoffs, reviewer ≠ owner, and completion that re-hashes its own evidence receipts. ~27k lines of dependency-free Python behind 1,335 passing tests — each design put through multi-round adversarial review by a rival model *before* any code, and the round that earned its keep found a live defect in already-shipped code that 295 passing tests had missed. The first supervised run earned its keep by failing usefully: agents sat idle with actionable work in their own inboxes, waiting to be re-invoked by an orchestrator that attended mode didn't have. That idle *was* the unsolved problem, hidden while a human was driving — so the acceptance gate moved to autonomous coordination, and a human board write mid-run now *fails* it. It has since run that way end-to-end: first on itself across three providers, then on a repo that isn't its own — fixing a real bug there, zero mid-run human writes, that repo's own test gate green on the result. Status, by the project's own ladder: **tested, and exercised** — existence, not reliability. Reliability is now measured rather than asserted: a pre-registered batch of ten heterogeneous trials — four foreign repos, three providers, the plan frozen to a commit before any run started — ran on 8 August at **8/10 correct terminal states, zero critical protocol violations**. What it caught matters more than the score. One trial's red arm refused to fake success and stopped for input rather than reporting done. One failure was the real product defect the batch existed to find — cold spawns overflowing the Windows argv limit — typed as a stall by the circuit breaker rather than spun on. The other was a runner killed mid-run: forensics excluded every other process on the machine with evidence and landed on my *own* operator harness — the measuring apparatus, not the system under test — and the row still stands as a counted failure, because voiding a pre-registered trial requires certainty. The ≥95% bar stays unclaimed until the clean-run expansion runs on the fixed commit; failed rows stay in the denominator.
